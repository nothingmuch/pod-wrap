...
This is a L<Pod::Parser> subclass, based on L<Pod::Stripper>. It parses perl files, wrapping pod text, and leaving everything else intact. It prints it's output to wherever you point it to (like you do with L<Pod::Parser> (and L<Pod::Stripper>)).
...
These are actually L<Pod::Parser/parse_from_filehandle> and L<Pod::Parser/parse_from_file>.

They will receive input and output as described therein.

They will invoke this parser class, which will filter the pod text, wrapping the appropriate sections using L<Text::Wrap>.
